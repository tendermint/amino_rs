...
fn amino_attrs(attrs: Vec<Attribute>) -> Result<Vec<Meta>, Error> {
    Ok(attrs.iter().flat_map(Attribute::interpret_meta).flat_map(|meta| match meta {
        Meta::List(MetaList { ident, nested, .. }) => if ident == "amino" {
        ...
        match attr {
            ...
            NestedMeta::Literal(lit) => bail!("invalid amino attribute: {:?}", lit),
        }
    }).collect())
}
